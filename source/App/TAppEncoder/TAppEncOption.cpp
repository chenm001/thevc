...
TAppOption::processFile()
{
  if( ! (valueStoreOK() && FileSet())  )
    return false;
  char *pbuffer = readFile();
  return  ( consumeFile( pbuffer ) );
}
...
void
TAppOption::processLine( char *theline, int ilength  )
{
  bool found = false;
  char *pline = (char*) malloc( (ilength+1)*sizeof(char) );
  for( int i = 0 ; i < ilength ; i ++ )
    pline[i]= *(theline++);
  pline[ilength] = nullterminate;
  char *cursor = pline ; /* preserve the ptr */
  if( *cursor == delimiter || *(cursor+ilength-1) == delimiter ){
    justValue( pline );/* line with start/end delimiter */
  }
  else
  {
    for( int i = 1 ; i < ilength-1 && !found ; i++){/* delimiter */
      if( *cursor == delimiter )
      {
